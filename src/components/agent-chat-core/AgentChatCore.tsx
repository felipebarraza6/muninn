...
interface AgentChatCoreProps {
  ...
  fillParent?: boolean;
  agentSwitcher?: {
    agents: import("@/api/hooks/useAgents").Agent[];
...
export function AgentChatCore({
  ...
  // Composer
  const composer = useAgentChatComposer({
    agentId,
    conversationId,
    isDraftNew,
    allFunctions,
  ...
  const streaming = useAgentChatStreaming({
    ...
    attachedSkills: composer.attachedSkills,
  ...
  useEffect(() => {
    streaming.conversationIdRef.current = conversationId;
  }, [conversationId]);
  ...
  const {
    ...
    showJump,
    scrollToBottom,
  } = useStickyChatScroll([messages, isBusy, streaming.isStreaming], {
    behavior: streaming.isStreaming ? "auto" : "smooth",
  });
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      if (e.deltaY < 0) typingPinRef.current = false;
      else if (e.deltaY > 0 && el.scrollHeight - el.scrollTop - el.clientHeight <= 96)
  ...
  useEffect(() => {
    if (!(streaming.isStreaming || streaming.liveSteps.length > 0 || typingActive)) {
      typingPinRef.current = true;
      return;
    }
    let rafId: number;
    const loop = () => {
      if (
        typingPinRef.current &&
        (streaming.isStreaming || streaming.liveSteps.length > 0 || hasActiveTyping())
      ) {
        scrollToBottom("auto");
  ...
  useEffect(() => {
    if (!initialScrollDoneRef.current && messages.length > 0 && !streaming.isStreaming) {
      const t = setTimeout(() => {
        scrollToBottom();
  ...
  useEffect(() => {
    ...
    if (conversationIdFromUrl && !streaming.skipAutoSelectRef.current) {
      setIsDraftNew(false);
  ...
  useEffect(() => {
    if (streaming.creatingConversationRef.current) {
  ...
  const chatPhase = deriveChatPhase({
    ...
    conversationId,
    isDraftNew,
    isCreating: streaming.isCreating,
    isStreaming: streaming.isStreaming,
    sendPending: sendMessage.isPending,
    error: streaming.createError,
  });
  ...
  const conversationStartedAt = useMemo(
    () => currentConversation?.created ?? messages.find((m) => m.created)?.created ?? null,
      ...
      allConversations
        .filter((c: UnifiedConversation) => {
          ...
          return historyTab === "active"
            ? s === "active"
            : s === "archived" || s === "closed" || s === "inactive";
        })
        .sort((a, b) => new Date(b.modified ?? 0).getTime() - new Date(a.modified ?? 0).getTime())
        .slice(0, 50),
  ...
  return (
    <div className={shellClass}>
      <header className="flex shrink-0 flex-col gap-2 border-b border-border/50 bg-card/50 px-4 py-3 backdrop-blur sm:flex-row sm:items-center sm:gap-3 sm:px-5">
          ...
          {!conversationId && (
            <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:hidden">
              <Button
                ...
                onClick={() => void streaming.handleNewConversation()}
                disabled={streaming.isCreating}
        ...
        <div className="flex items-center gap-1.5 sm:ml-auto sm:gap-2">
          ...
          {conversationId && (
            <>
              <Button
                ...
                onClick={() => streaming.setEscalateOpen(true)}
                disabled={streaming.escalateConversation.isPending}
              ...
              <Button
                ...
                size="sm"
                className="shrink-0 gap-1.5 cursor-pointer"
                onClick={() => streaming.setConfirmCloseOpen(true)}
                disabled={streaming.closeConversation.isPending || streaming.updateStatus.isPending}
          ...
          <AgentChatHistorySheet
            ...
            updatePending={streaming.updateStatus.isPending}
            onSelect={streaming.handleSelectConversation}
            onArchive={streaming.handleArchiveConversation}
            onRestore={streaming.handleRestoreConversation}
          />
        </div>
      </header>
      ...
      >
        ...
        {chatPhase === "resolving_thread" || chatPhase === "loading_history" ? (
          ...
          <EmptyState
            ...
            title={streaming.isCreating ? "Iniciando conversación…" : `Chatea con ${agent.name}`}
            description={
              streaming.isCreating
                  ...
                  {["¿Qué puedes hacer?", "Resume tu conocimiento", "Prueba una skill con /"].map(
                    (p) => (
                      <Button
                        ...
                        size="sm"
                        className="text-xs"
                        onClick={() => void streaming.handleSend(undefined, { text: p })}
                      >
                        {p}
                      </Button>
                    ),
          ...
          messages.map((msg, idx) => {
            const isStreamingBubble =
              ...
              String(msg.id) === streaming.streamingMessageId;
            return (
              <AgentChatMessage
                key={msg.id}
                msg={msg}
                ...
                isBusy={isBusy}
                agent={agent}
                liveSteps={isStreamingBubble ? streaming.liveSteps : EMPTY_LIVE_STEPS}
                skipEntrance={streaming.historyIdsRef.current.has(String(msg.id))}
                onReply={composer.startReply}
                onResend={streaming.resendMessage}
      ...
      {streaming.createError && (
        <ErrorBanner
          variant="bar"
          message={streaming.createError}
          onRetry={() => void streaming.handleNewConversation()}
        />
      )}
      ...
      <div className="border-t border-border/50 bg-card/50 backdrop-blur px-4 py-3 sm:px-5 sm:py-4">
        <div className="max-w-3xl mx-auto">
          <ChatSkillCommand
            open={composer.skillMenuOpen && agentSkillOptions.length > 0}
            onOpenChange={composer.setSkillMenuOpen}
            skills={agentSkillOptions}
            query={composer.slashQuery?.query ?? ""}
            onSelect={composer.selectSkillCommand}
          >
            <ChatComposer
              ref={composer.textareaRef}
              value={composer.input}
              onChange={composer.setInput}
              onCursorChange={composer.setInputCursor}
              onSubmit={() => void streaming.handleSend(undefined, { text: composer.input })}
              placeholder={
                streaming.isCreating
                  ...
                  : isBusy
                    ...
                    : composer.replyTo
                      ...
                      : composer.attachedSkills.length > 0
              ...
              busy={isBusy}
              canStop={streaming.isStreaming}
              onStop={streaming.stopStreaming}
              canSubmit={composer.attachedSkills.length > 0}
              leading={
                composer.replyTo || composer.attachedSkills.length > 0 ? (
                  <div className="space-y-2">
                    {composer.replyTo && (
                      <div className="flex items-start gap-2 rounded-xl border border-primary/25 bg-primary/5 px-3 py-2">
                        <div className="min-w-0 flex-1">
                          <p className="text-[11px] font-semibold text-primary">
                            Respondiendo a {composer.replyTo.role === "user" ? "ti" : "agente"}
                          </p>
                          <p className="truncate text-xs text-muted-foreground">
                            {composer.replyTo.preview}
                          </p>
                        </div>
                        <button
                          type="button"
                        ...
                        {composer.attachedSkills.map((s) => {
                          ...
                          return (
                            <span
                              ...
                              title={p.length ? `${s.name}: ${p.join(", ")}` : s.name}
                            >
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  composer.setAttachedSkills((prev) =>
      ...
      <Dialog
        open={Boolean(composer.pendingSkill)}
        onOpenChange={(o) => {
          if (!o) composer.setPendingSkill(null);
        }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              ...
              Parámetros de {composer.pendingSkill?.skill.name ?? "skill"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1">
            ...
            {composer.pendingSkill?.fields.map((f) => (
              <div key={f.key} className="space-y-1.5">
                <Label htmlFor={`skill-param-${f.key}`} className="text-xs capitalize">
                  {f.label}
                </Label>
                <Input
                  id={`skill-param-${f.key}`}
                  value={composer.pendingSkill?.values[f.key] ?? ""}
                  onChange={(e) =>
                    composer.setPendingSkill((prev) =>
                      prev
                        ? { ...prev, values: { ...prev.values, [f.key]: e.target.value } }
                        : prev,
                    )
                  }
                  placeholder={f.description || f.key}
                  className="h-9"
                  autoFocus={f.key === composer.pendingSkill?.fields[0]?.key}
                />
              </div>
            ))}
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button type="button" variant="ghost" onClick={() => composer.setPendingSkill(null)}>
              Cancelar
            </Button>
            <Button type="button" onClick={composer.confirmPendingSkill}>
              Añadir skill
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        open={streaming.escalateOpen}
        onOpenChange={(o) => {
          streaming.setEscalateOpen(o);
          if (!o) streaming.setEscalateReason("");
        }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-2 py-1">
            ...
            <Input
              id="escalate-reason"
              value={streaming.escalateReason}
              onChange={(e) => streaming.setEscalateReason(e.target.value)}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  streaming.handleEscalateCurrent();
                }
              }}
            />
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button type="button" variant="ghost" onClick={() => streaming.setEscalateOpen(false)}>
              Cancelar
            </Button>
            <Button
              type="button"
              disabled={
                !streaming.escalateReason.trim() || streaming.escalateConversation.isPending
              }
              onClick={streaming.handleEscalateCurrent}
            >
              {streaming.escalateConversation.isPending && (
      ...
      <ConfirmDialog
        open={streaming.confirmCloseOpen}
        onOpenChange={streaming.setConfirmCloseOpen}
        ...
        busy={streaming.closeConversation.isPending || streaming.updateStatus.isPending}
