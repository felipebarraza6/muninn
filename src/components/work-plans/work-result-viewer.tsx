...
export function WorkResultViewer({
  ...
  if (error) {
    return (
      <div
        className={cn(
          "rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-3 text-sm text-destructive whitespace-pre-wrap",
          className,
        )}
      >
        {error}
  ...
  if (!text.trim() && !objectEntries) {
    return (
      <p
        className={cn(
          "text-xs text-muted-foreground rounded-xl border border-dashed border-border/80 px-3 py-6 text-center",
          className,
        )}
      >
        {emptyHint}
